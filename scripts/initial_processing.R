# Pre-proccessing functions for the taxonomic tables, before further analyses
...
format_count_table <- function(tb) {
  ...
  if (length(select(tb, starts_with("taxonReads") & !ends_with("_M"))) != 0) {
    tb <- tb %>% rename_with(
      .cols = starts_with("taxonReads") & !ends_with("_M"),
      .fn = ~ paste0(.x, "_M")
    )
  }
  return(tb)
...
calculate_clade_counts <- function(tb) {
  ...
  return(list(raw_taxon = tb_raw_taxon, raw_clade = tb_raw_clade))
}
  ...
  for (nm in genera_names) {
    ...
    tb <- tb %>%
      mutate(taxLineage = case_when(
        str_detect(taxLineage, unclass_nm) ~ gsub(
          str_c(unclass_nm, ">"),
          "", taxLineage
        ),
        TRUE ~ taxLineage
...
calc_clade_counts <- function(tb) {
  # Calculates clade counts (cumulative read counts of all taxa within a given
  # clade). Implemented with tree data structure/packages to make runtime
  # efficient.
  ...
  tb_raw_taxon$pathString <- str_replace_all(
    tb_raw_taxon$taxLineage, "/", # must be path string
    "-"
  )

  tb_raw_taxon$pathString <- str_replace_all(
    tb_raw_taxon$pathString, ">", # must be path string
    "/"
  )
  ...
  for (colName in truncated_colNames) {
    tb_tree$Do(function(node) {
      node$Aggregation <- sum(
        if (is.na(GetAttribute(node, attribute = colName))) {
          0
        } else {
          GetAttribute(node, attribute = colName)
        },
        if (node$isLeaf) {
          0
        } else {
          data.tree::Aggregate(node, attribute = "Aggregation", aggFun = sum)
        }
      )
